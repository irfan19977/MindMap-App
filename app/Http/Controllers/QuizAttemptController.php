...
class QuizAttemptController extends Controller
{
    ...
    public function submit(Request $request)
    {
        ...
        // First, calculate score without saving
        $totalPoints = 0;
        $earnedPoints = 0;
        $answersData = [];
        ...
        // If not passed, return score without saving
        if ($status === 'failed') {
            // Delete the attempt since user didn't pass
            QuizAnswer::where('quiz_attempt_id', $attempt->id)->delete();
            $attempt->delete();

            return response()->json([
                'success' => true,
                'message' => 'Quiz tidak lulus',
        ...
        try {
            ...
            // Update user progress and unlock next material
            ...
            // Find and unlock next material in the same subcategory
            $nextMaterial = Material::where('subcategory_id', $material->subcategory_id)
                ->where('status', 'publish')
                ->when($material->order_number !== null, function($query) use ($material) {
                    return $query->where('order_number', '>', $material->order_number);
                })
                ->orderBy('order_number', 'asc')
                ->first();

            $unlockedNextMaterial = false;
            if ($nextMaterial) {
                // Create user progress entry for next material (unlocks it)
            ...
            return response()->json([
                ...
                'passing_score' => $quiz->passing_score,
                'saved' => true,
                'unlocked_next_material' => $unlockedNextMaterial,
